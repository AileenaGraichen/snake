...
function createInitialModel() {
  let m = Array(10).fill(null).map(() => Array(10).fill(0));
...
function updateDirection(event) {
  ...
  if (validKeys.includes(event.key) && !isOppositeDirection(event.key, lastDirection)) {
    lastDirection = event.key;
  }
}

function isOppositeDirection(newDirection, lastDirection) {
  return newDirection === "ArrowLeft" && lastDirection === "ArrowRight" ||
